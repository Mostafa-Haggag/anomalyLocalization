    ...
    def __init__(self, multiplier, size, num_blocks=2, num_layers_per_block=4, is_top=False) -> None:
        ...
        if is_top:
            self.block_list = [EncoderBlock(
                in_channels=multiplier*size, out_channels=multiplier*size, num_layers=num_layers_per_block)
...
class Encoder(nn.Module):
    def __init__(self, num_tokens=8192,
        ...
        top_encoder_group = EncoderGroup(
            multiplier=groups_multiplier[0], size=hidden_base, num_blocks=num_blks_per_grp,
            num_layers_per_block=num_layers_per_blk, is_top=True)

        encoder_rest_group = [EncoderGroup(multiplier=m, size=hidden_base, num_blocks=num_blks_per_grp,
                                           num_layers_per_block=num_layers_per_blk)
...
class DecoderGroup(nn.Module):
    def __init__(self, multiplier, size, num_blocks=2, num_layers_per_block=4, n_init=None) -> None:
        ...
        if n_init is not None:
            ...
            rest_block_list = [DecoderBlock(
                in_channels=decreased_size, out_channels=decreased_size, num_layers=num_layers_per_block)
                for i in range(num_blocks - 1)]
...
class Decoder(nn.Module):
    def __init__(self, hidden_base=256, codebook_dim=1024, num_groups=4, n_init=128, num_blocks_per_group=2,
                 num_layers_per_block=4) -> None:
        super().__init__()
        ...
        top_group = DecoderGroup(
            multiplier=groups_multiplier[0], size=hidden_base, num_blocks=num_blocks_per_group, n_init=n_init,
            num_layers_per_block=num_layers_per_block)

        rest_group = [DecoderGroup(multiplier=m, size=hidden_base, num_blocks=num_blocks_per_group,
                                   num_layers_per_block=num_layers_per_block)
                      for m in groups_multiplier[:-1]]
...
class DiscreteVAE(nn.Module):
    def __init__(
            ...
            normalization=((0.5,) * 3, (0.5,) * 3)# this is normalization instead of network
    ):
        super().__init__()
        # This is very interesting  why do I need to have log2? what can be done ?
        assert log2(image_size).is_integer(), 'image size must be a power of 2'
        assert num_layers >= 1, 'number of layers must be greater than or equal to 1'
        # I am passing 0 so this will be False
        has_resblocks = num_resnet_blocks > 0

        self.image_size = image_size # set to 256
        self.num_tokens = num_tokens # set to 8192
        self.num_layers = num_layers # set to False
        self.temperature = temperature # set to 0.9
        self.straight_through = straight_through # set to False
        self.codebook = nn.Embedding(num_tokens, codebook_dim) # this is the code book
        # I set num_tokens to 512
        # I set codebook_dim to 2048

        hdim = hidden_dim# set to 64

        enc_chans = [hidden_dim] * num_layers # by multypling you  create a list of size of [64,64,64,64,64,64]
        dec_chans = list(reversed(enc_chans)) # you have the same number of chennales in the encoder

        enc_chans = [channels, *enc_chans] # you are concating the first channel containing (3) then you add all the
        # other channels
        # dec_init_chan = codebook_dim if you donot have reblocks (This is the case) else you have hidden_dim
        dec_init_chan = codebook_dim if not has_resblocks else dec_chans[0]
        dec_chans = [dec_init_chan, *dec_chans] # [codebook_dim,dec_chans] or [hidden_dim,dec_chans]
        # enc_chans contains [3,64]
        # dec_chans contains [2048, 64]
        enc_chans_io, dec_chans_io = map(lambda t: list(zip(t[:-1], t[1:])), (enc_chans, dec_chans))
        '''
            enc_chans = [3,64,64,64]
            dec_chans = [2048,64,64,64]
            Input has size: [(3, 64), (64, 64), (64, 64)]
            Output has size: [(2048, 64), (64, 64), (64, 64)]
        '''
        enc_layers = []
        dec_layers = []
        ...
        # This is zero so you donot inter
        for _ in range(num_resnet_blocks):
            dec_layers.insert(0, ResBlock(dec_chans[1]))  # insert a loaction 0 in here !
            # The insert() method in Python lists is used to insert an element at a specific position.
            enc_layers.append(ResBlock(enc_chans[-1]))

        if num_resnet_blocks > 0:
            dec_layers.insert(0, nn.Conv2d(codebook_dim, dec_chans[1], 1))
            # 1d conv to decrease the number of channnels
    ...
    def norm(self, images): # This has similar idea to what we did to normalize everything
    ...
    def forward(
        ...
        device, num_tokens, image_size, kl_div_loss_weight = img.device, self.num_tokens, self.image_size,\
        ...
        # this is the temperature of the soft_one_hot
        temp = default(temp, self.temperature)
        # straight_through set to False
...
class DiscreteVAE2(nn.Module):
    def __init__(self, num_tokens=8192, codebook_dim=2048,
                 ...
                 num_decoder_init=128, temperature=0.9, reconstrution_loss='smooth_l1_loss', kl_div_loss_weight=0.1,
                 logit_laplace_eps=None) -> None:
        super().__init__()
        ...
        self._decoder = Decoder(hidden_base=hidden_base, codebook_dim=codebook_dim, num_groups=num_groups,
                                n_init=num_decoder_init, num_blocks_per_group=num_blocks_per_group,
                                num_layers_per_block=num_layers_per_block)
